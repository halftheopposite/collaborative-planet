...
// --- Perlin Noise (3D) Utilities for initial terrain ---
// Improved Perlin noise permutation table (fixed for determinism)
...
function fbm3JS(x: number, y: number, z: number, octaves = 3): number {
  let value = 0;
  let amplitude = 0.5;
  let frequency = 1.0;
...
function createPlanet() {
  ...
  // --- Gentle initial terrain via seamless 3D Perlin fBM ---
  ...
  // Stronger: continents + mid + high-frequency + subtle ridged component
  const freq1 = 0.8; // very low frequency (large continents)
  const freq2 = 2.6; // medium details
  const freq3 = 6.0; // fine details
  const freqR = 3.0; // ridged frequency
  const amp1 = 3.2; // world units
  const amp2 = 1.8; // world units
  const amp3 = 0.75; // world units (fine details)
  const ampR = 0.9; // world units (ridged, zero-mean)
  // Localized extra detail mask and layer (adds noise only at some places)
  const maskFreq = 0.45;
  const maskEdge0 = 0.55;
  const maskEdge1 = 0.72; // higher => sparser spots
  const extraFreq = 8.0;
  const extraAmp = 0.6;
  for (let i = 0; i < normalAttr.count; i++) {
    ...
    // Sample fBM at two scales for variety; use unit normal for seamless sampling
    ...
    if (mask > 0.0) {
      ...
      h += extra * (extraAmp * (mask * mask)); // square mask to keep it subtle at edges
    }
    // Mild shaping to enhance forms without harsh peaks
    const sign = h < 0 ? -1 : 1;
